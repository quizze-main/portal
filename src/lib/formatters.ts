...
// Форматирование числа с пробелами (русская локаль), округление до целых
...
// V2: поддержка настраиваемых thresholds + подсветка для forecast метрик
export const calculateMetricStatus = (
  ...
  thresholds?: { critical?: number; good?: number }
): 'good' | 'warning' | 'critical' => {
  if (forecastLabel === 'deviation' && forecastValue !== undefined) {
    const critVal = thresholds?.critical ?? -5;
    const goodVal = thresholds?.good ?? 5;
    if (forecastValue <= critVal) return 'critical';
    if (forecastValue >= goodVal) return 'good';
    return 'warning';
  }

  // V2: forecast метрики тоже получают статус через thresholds
  if (forecastLabel === 'forecast' && forecastValue !== undefined && forecastValue > 0) {
