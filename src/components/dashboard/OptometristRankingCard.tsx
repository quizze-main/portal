...
export function OptometristRankingCard({ optometrist, showPercentFirst, highlightedField, onClick }: OptometristRankingCardProps) {
  ...
  return (
    <div 
      className="relative bg-card rounded-xl border border-border/80 shadow-md p-2 hover:bg-primary/5 transition-colors"
    >
      ...
      {onClick && (
        <button
          ...
          className="absolute top-2 right-2 h-8 w-8 rounded-full bg-background/80 backdrop-blur border border-border shadow-sm flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-background active:scale-95 transition"
          aria-label="Открыть"
          title="Открыть"
        >
