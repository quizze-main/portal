...
export interface RevenueSzRankingRow {
  id: string;
  rank: number;
  name: string;
  role: string;
  ...
  ordersCount: MetricIndicator;
  clientsCount: MetricIndicator;
  avgCost: MetricIndicator;
  conversion: MetricIndicator;
  lostRevenue: number;
}

export type RevenueSzSortField = 'name' | 'planPercent' | 'revenueSz' | 'ordersCount' | 
                  'clientsCount' | 'avgCost' | 'conversion' | 'lostRevenue';

interface RevenueSzRankingCardProps {
  manager: RevenueSzRankingRow;
  showPercentFirst: boolean;
  highlightedField?: RevenueSzSortField;
  onClick?: () => void;
}
...
const fieldToLabel: Record<RevenueSzSortField, string> = {
  name: '',
  planPercent: '',
  revenueSz: 'Выручка СЗ',
  ordersCount: 'Заказов СЗ',
...
export function RevenueSzRankingCard({ manager, showPercentFirst, highlightedField, onClick }: RevenueSzRankingCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Metrics Grid - 3 columns, 2 rows = 6 metrics */}
      <div className="grid grid-cols-3 gap-1">
        ...
        <CompactMetric 
          label="Заказов СЗ" 
          metric={manager.ordersCount} 
          formatValue={(v) => String(v)} 
          suffix=" шт"
          showPercentFirst={showPercentFirst}
          isHighlighted={isHighlighted('Заказов СЗ')}
        />
        <CompactMetric 
          label="Кол-во ФЛ" 
