...
import { avgPriceManagersDataByPeriod } from '@/data/periodData';
import { FilterPeriod } from './FilterBar';
import { formatNumber } from '@/lib/formatters';
import { useIsMobile } from '@/hooks/use-mobile';
import { AvgPriceRankingMobile, AvgPriceSortField, AvgPriceManagerRow } from './AvgPriceRankingMobile';
...
interface AvgPriceRankingTableProps {
  period?: FilterPeriod;
  onManagerClick?: (managerId: string) => void;
}

export function AvgPriceRankingTable({ period = 'month', onManagerClick }: AvgPriceRankingTableProps) {
  ...
  const handleSort = (field: AvgPriceSortField) => {
    setSortConfig(prev => ({
      field,
  ...
  const rows: AvgPriceManagerRow[] = useMemo(() => {
    const dataSource = avgPriceManagersDataByPeriod[period] || avgPriceManagersData;
    const managerRows = dataSource.map(manager => ({
      ...
      glassesComplete: manager.glassesComplete,
      frame: manager.frame,
      manufacturing: manager.manufacturing,
      lens: manager.lens,
      designShare: manager.designShare,
      designLens: manager.designLens,
    ...
    managerRows.sort((a, b) => {
      ...
      if (['glassesComplete', 'frame', 'lens', 'designShare', 'designLens'].includes(field)) {
        const metricA = a[field as keyof AvgPriceManagerRow] as MetricValue;
        const metricB = b[field as keyof AvgPriceManagerRow] as MetricValue;
        aVal = showPercentFirst ? metricA.forecast : metricA.value;
        bVal = showPercentFirst ? metricB.forecast : metricB.value;
      } else {
        aVal = a[field as keyof AvgPriceManagerRow] as number;
        bVal = b[field as keyof AvgPriceManagerRow] as number;
      }
  ...
  return (
    <div className="bg-card rounded-xl border shadow-sm overflow-hidden relative isolate">
      <div className="px-4 py-3 border-b bg-muted/60 flex items-center justify-between">
        <h3 className="font-semibold text-foreground">По менеджерам — Средняя стоимость</h3>
        <Button
          variant="ghost"
          size="sm"
          ...
          <tbody>
            {rows.map((row, index) => (
              ...
              >
                ...
                {/* Avg glasses */}
                <MetricCell metric={row.glassesComplete} formatValue={formatNumber} showPercentFirst={showPercentFirst} />
                
                {/* Avg frame */}
                <MetricCell metric={row.frame} formatValue={formatNumber} showPercentFirst={showPercentFirst} />
                
                {/* Avg lens */}
                <MetricCell metric={row.lens} formatValue={formatNumber} showPercentFirst={showPercentFirst} />
                
                {/* Design share */}
                <MetricCell metric={row.designShare} formatValue={(v) => String(v)} showPercentFirst={showPercentFirst} suffix="%" />
                
                {/* Avg design lens */}
                <MetricCell metric={row.designLens} formatValue={formatNumber} showPercentFirst={showPercentFirst} />
