...
// Уровни выполнения личного плана менеджера
export const MANAGER_LEVELS = ['<100%', '100%', '110%', '120%', '130%'] as const;
export type ManagerLevel = typeof MANAGER_LEVELS[number];

// Уровни выполнения плана клуба
export const CLUB_LEVELS = ['<95%', '100%', '110%>'] as const;
export type ClubLevel = typeof CLUB_LEVELS[number];

// Тип матрицы премий 3×5
export type MotivationMatrixType = Record<ClubLevel, Record<ManagerLevel, number>>;
...
export interface KPIConfig {
  ...
  linkedMetricId?: string;        // Привязка к метрике дашборда (авто-расчёт tier)
  tiers: KPITier[];
}
...
export const DEFAULT_KPI: KPIConfig[] = [
  {
    ...
    description: 'С момента готовности до 10 дней',
    tiers: [
    ...
    kpis: [
      {
        ...
        description: 'Количество проданных линз с дизайном',
        tiers: [
      ...
      },
      {
        ...
        description: 'Процент заказов закрытых вовремя',
        tiers: [
          { range: '90-100%', bonus: 5000, minPercent: 90, maxPercent: 100 },
    ...
    kpis: [
      {
        ...
        description: 'Бонус за линзы с дизайном',
        tiers: [
    ...
    kpis: [
      {
        id: 'closure_time',
        label: 'Время закрытия заказа менее 10 дней',
        description: 'Время закрытия заказа менее 10 дней',
        type: 'tier',
        tiers: [
    ...
    kpis: [
      {
        ...
        description: 'Бонус за выполнение плана по линзам',
        type: 'tier',
        tiers: [
          { range: 'Выполнено', bonus: 10000, minPercent: 100, maxPercent: 100 },
          { range: 'Не выполнено', bonus: 0, minPercent: 0, maxPercent: 100 },
      ...
      },
      {
        id: 'checks',
        label: 'Проверки',
        description: '100 ₽ за каждую проверку',
        type: 'multiplier',
        multiplierRate: 100,
...
export function getManagerLevel(percent: number): ManagerLevel {
  if (percent >= 130) return '130%';
...
export function getClubLevel(percent: number): ClubLevel {
  if (percent >= 110) return '110%>';
  if (percent >= 95) return '100%';
  return '<95%';
}

// Минимальный процент для достижения уровня менеджера
export function getMinPercentForLevel(level: ManagerLevel): number {
  switch (level) {
    case '130%': return 130;
    case '120%': return 120;
    case '110%': return 110;
...
export function getTypicalPercentForClubLevel(level: ClubLevel): number {
  switch (level) {
    case '110%>': return 110; // 110%+ → минимум для этого уровня
    case '100%': return 100;  // 95-109% → середина диапазона
    case '<95%': return 90;   // 0-94% → реалистичный минимум
    default: return 100;
  }
}

// Проверка, что % клуба соответствует уровню матрицы
export function isClubPercentValidForLevel(percent: number, level: ClubLevel): boolean {
  switch (level) {
    case '110%>': return percent >= 110;
    case '100%': return percent >= 95 && percent < 110;
    case '<95%': return percent < 95;
...
export function calculateBonus(
  ...
  const managerLevel = getManagerLevel(personalPercent);
  const clubLevel = getClubLevel(clubPercent);
...
export function calculateSalary(params: {
  ...
  return {
    ...
    managerLevel: getManagerLevel(personalPercent),
    clubLevel: getClubLevel(params.clubPercent),
  };
}
...
export function calculateRequiredPlanSmart(params: {
  ...
  // Максимально возможная зарплата (200% личный, 110% клуб, все KPI)
  const maxPersonalFact = personalPlan * 2.0;
  const maxBonusPercent = matrix['110%>']['130%'];
  ...
  const clubLevelPriority: ClubLevel[] = ['110%>', '100%', '<95%'];
  
  for (const clubLevel of clubLevelPriority) {
    const clubPercent = getTypicalPercentForClubLevel(clubLevel);
    
    for (const managerLevel of MANAGER_LEVELS) {
      const bonusPercent = matrix[clubLevel][managerLevel];
      const minPersonalPercent = getMinPercentForLevel(managerLevel);
      ...
      for (const kpisRecord of kpiCombinations) {
        ...
        if (requiredBonusAmount <= 0) {
          ...
          solutions.push({
            ...
            kpis: kpisRecord,
            kpiBonus,
            effort: calculateEffort(defaultPercent, clubLevel),
          });
          continue;
        }
        ...
        if (actualPersonalPercent >= minPersonalPercent && actualPersonalPercent <= 200) {
          const actualManagerLevel = getManagerLevel(actualPersonalPercent);
          const isValidLevel = managerLevel === '130%' 
            ? actualPersonalPercent >= 130 
          ...
          if (isValidLevel) {
            solutions.push({
              ...
              kpis: kpisRecord,
              kpiBonus,
              effort: calculateEffort(actualPersonalPercent, clubLevel),
            });
          }
        }
      }
...
export function calculateMatchingCells(params: {
  ...
  for (const clubLevel of CLUB_LEVELS) {
    for (const managerLevel of MANAGER_LEVELS) {
      ...
      const minPercent = getMinPercentForLevel(managerLevel);
      const maxPercent = managerLevel === '130%' ? 200 : getMinPercentForLevel(
        MANAGER_LEVELS[MANAGER_LEVELS.indexOf(managerLevel) + 1] as ManagerLevel
      ) - 0.01;
      
      const actualMaxPercent = managerLevel === '130%' ? 200 : maxPercent;
      
      if (requiredPersonalPercent >= minPercent && requiredPersonalPercent <= actualMaxPercent) {
        let effort = requiredPersonalPercent;
        if (clubLevel === '110%>') effort -= 20;
        else if (clubLevel === '<95%') effort += 15;
...
export function calculateBalancedPlan(params: {
  ...
  if (requiredBonusAmount <= 0) {
    ...
    return {
      ...
      clubFact: clubPlan * clubPercent / 100,
      managerLevel: getManagerLevel(personalPercent),
      clubLevel: getClubLevel(clubPercent),
      bonusPercent: matrix[getClubLevel(clubPercent)][getManagerLevel(personalPercent)],
      isAchievable: true,
    };
  }
  ...
  if (fixedPersonalPercent !== undefined) {
    const managerLevel = getManagerLevel(fixedPersonalPercent);
    const personalFact = personalPlan * fixedPersonalPercent / 100;

    for (const clubLevel of ['<95%', '100%', '110%>'] as ClubLevel[]) {
      const bonusPercent = matrix[clubLevel][managerLevel];
      const actualBonus = personalFact * (bonusPercent / 100);

      if (actualBonus >= requiredBonusAmount) {
        let clubPercent: number;
        if (clubLevel === '<95%') {
          clubPercent = 80;
        } else if (clubLevel === '100%') {
          clubPercent = 95;
        } else {
          clubPercent = 110;
        }
  ...
  if (fixedClubPercent !== undefined) {
    const clubLevel = getClubLevel(fixedClubPercent);
    const clubFact = clubPlan * fixedClubPercent / 100;

    for (const managerLevel of ['<100%', '100%', '110%', '120%', '130%'] as ManagerLevel[]) {
      const bonusPercent = matrix[clubLevel][managerLevel];
      ...
      const minPercent = getMinPercentForLevel(managerLevel);
      const maxPercent = managerLevel === '130%' ? 200 : 
        getMinPercentForLevel(MANAGER_LEVELS[MANAGER_LEVELS.indexOf(managerLevel) + 1] as ManagerLevel) - 0.01;

      if (requiredPersonalPercent >= minPercent && requiredPersonalPercent <= (managerLevel === '130%' ? 200 : maxPercent)) {
        return {
...
export function findBestCombinationWithKPI(params: {
  ...
  const personalMin = fixedPersonalPercent !== undefined ? fixedPersonalPercent : 50;
  const personalMax = fixedPersonalPercent !== undefined ? fixedPersonalPercent : 200;
  const personalStep = fixedPersonalPercent !== undefined ? 1 : 5; // Грубый шаг для 3D
  
  const clubMin = fixedClubPercent !== undefined ? fixedClubPercent : 90;
  const clubMax = fixedClubPercent !== undefined ? fixedClubPercent : 130;
...
/**
 * Получить максимальный процент для уровня менеджера
 */
export function getMaxPercentForLevel(level: ManagerLevel): number {
  switch (level) {
    case '<100%': return 99.99;
    case '100%': return 109.99;
    case '110%': return 119.99;
    case '120%': return 129.99;
    case '130%': return 200; // максимум
    default: return 200;
  }
}
...
export function findBestCombinationForTarget(params: {
  ...
  for (let club = 90; club <= 130; club += 1) {
    for (let personal = 50; personal <= 200; personal += 1) {
      const breakdown = calculateSalary({
        personalFact: personalPlan * personal / 100,
        personalPlan,
        clubPercent: club,
  ...
  for (let club = bestResult.club - searchRange; club <= bestResult.club + searchRange; club += 0.5) {
    for (let personal = bestResult.personal - searchRange; personal <= bestResult.personal + searchRange; personal += 0.1) {
...
export function calculateExactPersonalPercent(params: {
  ...
  for (const managerLevel of MANAGER_LEVELS) {
    const bonusPercent = matrix[clubLevel][managerLevel];
    const minPercent = getMinPercentForLevel(managerLevel);
    const maxPercent = getMaxPercentForLevel(managerLevel);
  ...
  const highestBonusPercent = matrix[clubLevel]['130%'];
  ...
  if (highestPercent >= 130) return highestPercent;
  
  const lowestBonusPercent = matrix[clubLevel]['<100%'];
